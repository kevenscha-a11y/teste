...
export function CoursePlayerPage() {
  ...
  if (error || !course) {
    return <div className="text-center py-20 text-destructive">Curso não encontrado.</div>;
  }
  ...
  const handleEnroll = async () => {
    try {
      ...
      toast({ title: "Matriculado com sucesso!", description: "Você já pode começar a aprender." });
    } catch (e: any) {
      toast({ variant: "destructive", title: "Erro na matrícula", description: e.message });
    }
  };
  ...
  const handleMarkComplete = async (lessonId: number) => {
    try {
      ...
      if (nextLessonId) {
        ...
        toast({ title: "Curso Concluído!", description: "Parabéns pela conquista!" });
      }
    } catch (e: any) {
      toast({ variant: "destructive", title: "Erro ao marcar como concluída", description: e.message });
    }
  };
        ...
        {course.isEnrolled && (
          <div className="flex items-center gap-4">
            <div className="hidden sm:flex flex-col items-end">
              <span className="text-xs font-semibold text-primary">{Math.round(course.progressPercent)}% Concluído</span>
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className="p-8 max-w-4xl w-full mx-auto">
                ...
                <div className="prose prose-invert max-w-none text-muted-foreground leading-relaxed">
                  {currentLesson.description || "Nenhuma descrição fornecida."}
                </div>
              </div>
            </div>
          ) : (
                ...
                <AccordionItem value={section.id.toString()} key={section.id} className="border-b border-border/50">
                  <AccordionTrigger className="px-4 py-4 hover:no-underline hover:bg-secondary/30 data-[state=open]:bg-secondary/10 transition-colors">
                    <div className="flex flex-col items-start text-left">
                      <span className="text-xs text-muted-foreground font-semibold uppercase tracking-wider mb-1">Seção {idx + 1}</span>
